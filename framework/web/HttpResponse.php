...
class HttpResponse extends BaseObject
{
    ...
    {
        $this->contentCharset = $config->Mvc->Charset;
    ...
    public function SetStatusCode(int $statusCode, ?string $statusText = null): void
    {
        ...
        $this->statusText = null === $statusText ?
        (isset(self::$httpStatus[$statusCode]) ? self::$httpStatus[$statusCode] : '') : $statusText;
    }
     ...
     * @param bool $httpOnly
     */
    public function AddCookie(string $name, ?string $value = null, ?int $expires = null, 
        ?string $path = null, ?string $domain = null, ?bool $secure = null, ?bool $httpOnly = null): void
    {
        $domain = null === $domain ? null : strtolower($domain);
        $this->cookie["{$domain}-{$path}-{$name}"] = new Cookie($name, $value, $expires, $path, $domain, $secure, $httpOnly);
    }
